package edu.umd.cs.bachuai13.timing
...
baseConfig.setProperty("admmreasoner.numthreads", 1);
...
List<Set<GroundingWrapper>> groundings = FoldUtils.splitGroundings(data, [trusts, knows], [fullTrusts, fullKnows], 8)
for (int i = 0; i < folds; i++) {
...
for (int fold = 0; fold < folds; fold++) {
	...
	for (int configIndex = 0; configIndex < configs.size(); configIndex++) {
		...
		config.setProperty("admmreasoner.numthreads", 1);
		for (CompatibilityKernel k : Iterables.filter(m.getKernels(), CompatibilityKernel.class))
